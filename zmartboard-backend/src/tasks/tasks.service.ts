  ...
  async moveTask(taskId: string, moveTaskDto: MoveTaskDto, userId: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      if (targetColumnId && targetColumnId !== sourceColumnId) {
        ...
        // Step 1: Move task to temporary position in target column
        ...
        // Step 2: Adjust positions in source column
        await tx.task.updateMany({
          where: {
        ...
        // Step 3: Make room in target column
        await tx.task.updateMany({
          where: {
            columnId: finalTargetColumnId,
        ...
        // Step 4: Move task to final position
        await tx.task.update({
          where: { id: taskId },
          data: {
        ...
        // Step 1: Move task to temporary position to avoid conflicts
        await tx.task.update({
          where: { id: taskId },
          data: { position: -1 },
        });
        ...
        // Step 3: Move task to final position
        await tx.task.update({
          where: { id: taskId },
